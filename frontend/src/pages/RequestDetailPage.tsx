...
export function RequestDetailPage() {
  ...
  useEffect(() => { if (request?.recurring) setDraft(request.recurring) }, [request?.recurring])
  ...
  async function previewRecurrence() { if (!request || !draft) return; try { setPreview(await api.previewRecurrence(request.id, { recurrence: draft })) } catch (e) { setMessage(e instanceof Error ? e.message : 'No se pudo calcular el impacto.') } }
  async function applyRecurrence(overwriteExceptions: boolean) { if (!request?.recurring || !draft) return; setApplying(true); try { await api.applyRecurrence(request.id, draft, overwriteExceptions); setPreview(undefined); setMessage('Recurrencia aplicada.'); await load() } catch (e) { setMessage(e instanceof Error ? e.message : 'No se pudo aplicar.') } finally { setApplying(false) } }
